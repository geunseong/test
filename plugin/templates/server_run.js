...
var console_msg = 'CONSOLE_MSG'; //when server running
var server_msg = 'SERVER_MSG'; //when server running
var handles;
  ...
  'delete_project' : function (browser) {
    browser
      ...
      .getText('#dlg_confirmation', function (result) {
        browser.verify.equal(/서버가|Server/.test(result.value), true, 'Element <#dlg_confirmation> has message contain "서버가" or "Server"');
        browser
